...
class Model(object):
    ...
    def _init_resource(self):
        ...
        #获取模型输出个数
        output_size = acl.mdl.get_num_outputs(self._model_desc)
        #创建模型输出dataset结构
    ...
    def _gen_output_tensor(self, output_size):
        for i in range(output_size):
            ...
            self._output_tensor.append({"ptr": tensor_ptr,
    ...
    def _output_dataset_to_numpy(self):
        ...
        for i in range(num):
            ...
            output_ptr = self._output_tensor[i]["ptr"]
            output_tensor = self._output_tensor[i]["tensor"]
