...
| **模型名称** | **模型说明**          | **模型下载路径**                                             |
| ------------ | --------------------- | ------------------------------------------------------------ |
| deploy_vel          | 基于tensorflow的去雾处理。 | 请参考https://github.com/Huawei-Ascend/modelzoo/tree/master/contrib/TensorFlow/Research/cv/dehaze/ATC_deploy_vel_tf_AE中README.md原始模型章节，下载**原始模型网络**及**模型权重文件**。 |
...
2. 运行可执行文件。
   ...
   - 如果是开发环境与运行环境分离部署，执行以下命令切换目录。
	```
     cd $HOME/python/SingleImageDehaze/src
	```
     切换目录后，执行以下命令运行样例。
