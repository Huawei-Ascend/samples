...
char* GetPicDevBuffer4JpegE(const PicDesc &picDesc, uint32_t &PicBufferSize)
{
    ...
    aclError aclRet = aclrtMallocHost(&inputHostBuff, PicBufferSize);
    if (aclRet !=  ACL_ERROR_NONE) {
        ERROR_LOG("malloc host data buffer failed, aclRet is %d", aclRet);
        return nullptr;
    }

    size_t readSize = fread(inputHostBuff, sizeof(char), PicBufferSize, fp);
    ...
    aclRet = aclrtMemcpy(inputDevBuff, PicBufferSize, inputHostBuff, PicBufferSize, ACL_MEMCPY_HOST_TO_DEVICE);
    if (aclRet != ACL_ERROR_NONE) {
...
int main()
{
    ...
    std::string encodeOutFileName = "./jpege_output";
